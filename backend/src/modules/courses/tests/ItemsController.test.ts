...
describe('Item Controller Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    // Register ProgressRepo and UserRepo in the container
    const progressRepo = new ProgressRepository(Container.get<MongoDatabase>('Database'));
    Container.set('ProgressRepo', progressRepo);
    const userRepo = new UserRepository(Container.get<MongoDatabase>('Database'));
