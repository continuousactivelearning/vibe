...
//     const prompt = `Your task is to analyze the following timed lecture transcript and segment it into meaningful subtopics.

// The transcript is a series of lines, each with a timestamp and text.
// Example line format: [start_time --> end_time] text

// Instructions:
// 1.  Group the transcript lines into logical subtopics.
// 2.  For each subtopic, create a JSON object.
// 3.  Each JSON object must have two keys: "end_time" and "segments".
// 4.  The "end_time" value must be the end timestamp from the *last* line of that subtopic.
// 5.  The "segments" value must be a JSON array containing the *original, unmodified transcript lines* for that subtopic.

// CRITICAL:
// - Your entire output MUST be a single, valid JSON array of these objects.
// - Do NOT include any text, explanations, or markdown (like \`\`\`json) before or after the JSON array.
// - Ensure all strings are enclosed in double quotes.
// - Ensure any double quotes inside the transcript text are properly escaped with a backslash (e.g., "he said \\"hello\\"").

// Example of a valid output object:
// {
//   "end_time": "01:30.000",
//   "segments": [
//     "[00:00.000 --> 00:30.000] First topic content",
//     "[00:30.000 --> 01:30.000] More content about the first topic"
//   ]
// }
...
//             if (!segment.end_time || !Array.isArray(segment.segments)) {
